...
export async function sendNewReviewEmail(params: NewReviewEmailParams) {
  const { to, businessName, reviewerName, starRating, reviewText, dashboardUrl } = params
  ...
  const subject =
    starRating >= 4
      ? `⭐ New ${starRating}-star review for ${businessName}`
      : `New ${starRating}-star review for ${businessName}`
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto;">
          <div style="padding: 32px 0 16px;">
            ...
            <p style="color: #6b7280; font-size: 14px; margin: 0;">${businessName}</p>
          </div>
          ...
          <div style="padding: 24px 0; text-align: center;">
            <a href="${dashboardUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">
              View in Dashboard
            </a>
          </div>
          ...
          <div style="border-top: 1px solid #e5e7eb; padding: 16px 0; text-align: center;">
            <p style="color: #9ca3af; font-size: 12px; margin: 0;">
              You're receiving this because you have review notifications enabled in ReviewFlow.
            </p>
          </div>
        </div>
      `,
    ...
    await getResend().emails.send({
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto;">
          ...
          <div style="border-top: 1px solid #e5e7eb; padding: 16px 0; text-align: center;">
            <p style="color: #9ca3af; font-size: 12px; margin: 0;">
              Weekly summaries are sent every Monday. Manage preferences in Settings.
            </p>
          </div>
        </div>
      `,
